    ...
    try {
        ...
        if (!req.user) {
            ...
            persons = persons.map(person => ({
                ...
                address: person.address ? person.address.split('省')[0] + '省...' : '' // 只显示省份
            }));
        } else {
            // 登录用户（包括普通用户和管理员）可以看到所有详细信息
            // 这是因为用户既可能是求职者也可能是招聘者，需要看到完整的人才信息
            persons = await databaseService.getAllPersonsWithDetails();
        }
...
const getPersonDetails = async (req, res) => {
    try {
        ...
        const personDetails = await databaseService.getPersonWithDetails(id);
...
// 创建或更新农村特色信息
const upsertRuralProfile = async (req, res) => {
    try {
...
const addSkill = async (req, res) => {
    try {
        ...
        const newSkill = await databaseService.addSkill(personId, skillData);
...
const searchTalents = async (req, res) => {
    try {
        ...
        const results = await databaseService.searchTalents(searchCriteria);
...
const getStatistics = async (req, res) => {
  try {
    ...
    // 获取基础统计
    const totalTalents = await databaseService.getTotalPersonsCount();
    const avgAge = await databaseService.getAverageAge();
    const totalSkills = await databaseService.getTotalSkillsCount();
    const cooperationStats = await databaseService.getCooperationStats();
    const skillsCategoryStats = await databaseService.getSkillsCategoryStats();
    const agricultureStats = await databaseService.getAgricultureStats();
    const educationStats = await databaseService.getEducationStats();
    const ageDistribution = await databaseService.getAgeDistribution();
...
// 获取技能特长库统计
const getSkillsLibraryStats = async (req, res) => {
  try {
    logger.info('Getting skills library statistics');
    
    const skillsLibrary = await databaseService.getSkillsLibraryStats();
    
    res.json(skillsLibrary);
  } catch (error) {
    logger.error('Error getting skills library stats:', error);
    res.status(500).json({ 
