        ...
        return new Promise((resolve, reject) => {
            ...
            db.serialize(() => {
                ...
                db.get("SELECT COUNT(*) as count FROM persons", (err, row) => {
                    ...
                    db.get("SELECT COUNT(*) as count FROM users WHERE role = 'admin'", (err, adminRow) => {
                        if (err) {
...
const getAllPersonsWithDetails = async () => {
    return new Promise((resolve, reject) => {
        ...
        // 首先获取所有基础人员信息
        db.all("SELECT * FROM persons ORDER BY id", async (err, persons) => {
            if (err) {
            ...
            try {
                ...
                const detailedPersons = await Promise.all(
                    persons.map(person => {
                        return new Promise((resolveDetail, rejectDetail) => {
                            // 获取农村特色信息
                            db.get("SELECT * FROM rural_talent_profile WHERE person_id = ?", [person.id], (err, ruralProfile) => {
                                if (err) {
                ...
                // 按ID排序
                detailedPersons.sort((a, b) => a.id - b.id);
...
const createPerson = async (personData) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        // 检查邮箱和手机号是否已存在
        db.get("SELECT id FROM persons WHERE email = ? OR phone = ?", [email, phone], (err, existingPerson) => {
            if (err) {
            ...
            db.run(`INSERT INTO persons 
                ...
                function(err) {
                    ...
                    // 获取新创建的人员信息
                    db.get("SELECT * FROM persons WHERE id = ?", [newPersonId], (err, newPerson) => {
                        if (err) {
    ...
    return new Promise((resolve, reject) => {
        ...
        db.get("SELECT * FROM persons WHERE id = ?", [id], (err, existingPerson) => {
            ...
            if (hasUniqueFields) {
                ...
                db.get(duplicateCheckSql, duplicateCheckParams, (err, duplicatePerson) => {
                    if (err) {
...
const getAverageAge = () => {
    return new Promise((resolve, reject) => {
        const db = createConnection();
        db.get('SELECT AVG(age) as avgAge FROM persons', (err, row) => {
            db.close();
            if (err) {
...
const getTotalSkillsCount = () => {
    return new Promise((resolve, reject) => {
        const db = createConnection();
        db.get('SELECT COUNT(*) as count FROM talent_skills', (err, row) => {
            db.close();
            if (err) {
...
const getAgricultureStats = () => {
    return new Promise((resolve, reject) => {
        ...
        // 获取平均农业经验年数
        db.get('SELECT AVG(farming_years) as avgFarmingYears FROM rural_talent_profile WHERE farming_years IS NOT NULL', (err, avgRow) => {
            if (err) {
                db.close();
